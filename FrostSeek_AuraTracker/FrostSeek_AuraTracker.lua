...
local function Scan(silent)
    ...
    if state.initializedRoster and
...
local function CreateUI(parent)
    ...
    cd:SetWidth(295)
    cd:SetJustifyH("LEFT")
    cd:SetText("Exact “aura” = Aura candidate. Other replies are listed separately.")
...
RefreshCandidates = function()
    ...
    local function RenderList(widget, source, auraList)
        ...
        for i, c in ipairs(list) do
            ...
            row.message:SetText(c.message or "")
            ...
            row.frame:SetScript("OnEnter", function(self)
                ...
                if auraList then
                    GameTooltip:AddLine("Exact “aura” reply", 0.4, 1.0, 0.4, true)
                else
...
FSA:SetScript("OnEvent", function(self, event, arg1, arg2)
    if event == "ADDON_LOADED" and arg1 == ADDON then
        ...
        CreateOverlay()
        RefreshOverlay()
        Print("Loaded Aura Tracker v1.1.3")
        state.pendingScan = 0.5

    elseif event == "PLAYER_LOGIN" then
        integrated = IntegrateWithFrostSeek()
        CreateOverlay()
    ...
    elseif event == "PLAYER_ENTERING_WORLD" or event == "ZONE_CHANGED_NEW_AREA" then
        ...
        if nowMS and not state.lastManastormState then
            state.manastormAuditDue = GetTime() + 2.5
        end
        state.lastManastormState = nowMS
        state.pendingScan = nowMS and 2.5 or 0.35
    ...
    elseif event == "CHAT_MSG_WHISPER" then
        ...
        if sender and RecruitmentReplyWindowOpen() then
            local k = Key(sender)
            local entry = {
                name = sender,
                message = message,
            ...
            if IsExactAuraReply(message) then
                entry.status = previous and previous.status or "New"
                state.candidates[k] = entry
                state.nonAuraCandidates[k] = nil
                if not duplicateReply then
                    Print("Aura candidate: " .. sender .. " whispered exact keyword \"aura\".")
                end

                if FrostSeekAuraDB.autoInviteAuraWhispers and entry.status ~= "Invited" then
                    InviteUnit(sender)
                ...
                state.candidates[k] = nil
                if not duplicateReply then
                    Print("Non-aura candidate: " .. sender .. " whispered \"" .. message .. "\".")
                end
            end
...
local function FrostSeekAuraSlashHandler(msg)
    ...
    elseif cmd == "debug" then
        ...
        Print("/fsaura show | overlay | scan | sensors | mark <name> | unmark <name> | forget [name] | recruit | debug")
    end
end
